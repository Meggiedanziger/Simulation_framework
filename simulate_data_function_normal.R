  ...
  sim_dat     <- data.frame(subj = 1:nsubj,
    ...
    for(i in 1:N) {
      if(sim_dat$group[i] == "control") {
        value[i] <- rnorm(1, mean = 0.00001, sd = 1) # for control group generate random value from this distrubution
      } else {
        value[i] <- rnorm(1, mean = 0, sd = 1) - d # for treatment group generate random value from this distrubution
      }
    }
